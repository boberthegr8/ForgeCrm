...
export interface LocalCustomerMatch {
  customer: any;
  score: number;
  reasons: string[];
}

export function findBestLocalCustomerMatch(draft: QuoteIntakeDraft): LocalCustomerMatch | undefined {
  try {
    const data = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');
    const incomingDisplay = normalize(draft.customerName);
    const incomingPhone = phoneDigits(draft.phone);
    const incomingAccount = clean(draft.customerNumber);
    const matches = (data.customers || []).map((customer: any) => {
      let score = 0;
      const reasons: string[] = [];
      if (incomingAccount && customer.externalCustomerNumber && String(customer.externalCustomerNumber) === incomingAccount) {
        score += 100;
        reasons.push('same customer/account number');
      }
      if (incomingPhone.length >= 10 && (customer.phones || []).some((phone: string) => phoneDigits(phone) === incomingPhone)) {
        score += 55;
        reasons.push('same phone');
      }
      const display = normalize(customer.company || `${customer.firstName || ''} ${customer.lastName || ''}`);
      if (incomingDisplay && display === incomingDisplay) {
        score += 45;
        reasons.push('same normalized customer name');
      }
      if (draft.address && customer.address && normalize(customer.address) === normalize(draft.address)) {
        score += 30;
        reasons.push('same address');
      }
      return { customer, score: Math.min(score, 100), reasons };
    }).filter((match: LocalCustomerMatch) => match.score > 0).sort((a: LocalCustomerMatch, b: LocalCustomerMatch) => b.score - a.score);
    return matches[0];
  } catch {
    return undefined;
  }
...
export function importQuoteDraftLocally(draft: QuoteIntakeDraft, customerIdOverride?: string): QuoteImportResult {
  const data = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');
  ...
  const bestMatch = findBestLocalCustomerMatch(draft);
  let customer = customerIdOverride ? data.customers.find((item: any) => item.id === customerIdOverride) : (bestMatch?.score || 0) >= 70 ? bestMatch?.customer : undefined;
  ...
  if (!customer) {
    ...
    if (draft.phone && !(customer.phones || []).some((phone: string) => phoneDigits(phone) === phoneDigits(draft.phone))) {
      customer.phones = [...(customer.phones || []), draft.phone];
    }
  ...
  const existingQuote = data.quotes.find((quote: any) => clean(quote.quoteNumber) === clean(draft.quoteNumber));
  let quote: any;
  let createdQuote = false;
  let createdRevision = false;
  if (existingQuote) {
    existingQuote.revisionHistory = Array.isArray(existingQuote.revisionHistory) ? existingQuote.revisionHistory : [];
    const nextVersion = Number(existingQuote.version || 1) + 1;
    existingQuote.revisionHistory.push({
      version: existingQuote.version || 1,
      capturedAt: new Date().toISOString(),
      totalValue: existingQuote.totalValue,
      scopeSummary: existingQuote.scopeSummary,
      sourceFileName: existingQuote.sourceFileName
    });
  ...
  return { customerId: customer.id, quoteId: quote.id, quoteNumber: quote.quoteNumber, createdCustomer, createdQuote, createdRevision };
}
